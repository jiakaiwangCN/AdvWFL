...
class Cluster:
    def __init__(self, file_path, min_clusters=2, max_clusters=5):
        ...
        self.data = np.array(file.iloc[:, :-2])
        self.site = np.array(file.iloc[:, -2:])
    ...
    def kmeans(self):
        ...
        df = pd.DataFrame(sorted_array, columns=[f'col_{i}' for i in range(1, 186)])
        groups = df.groupby(df.iloc[:, -1])
        k_arrays = [group.values for _, group in groups]

        array_name = [f'RSS{i}' for i in range(1, 183)] + ['x', 'y']
        new_file_names = []
