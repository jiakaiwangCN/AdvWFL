...
def get_error(predict, abs, dic):
    ...
    for i in range(len(abs)):
        ...
        err += sqrt((x_abs - x_predict) ** 2 + (y_abs - y_predict) ** 2)
    return err / len(predict)


def get_error_list(predict, abs, dic):
    predict = torch.nn.functional.softmax(predict, dim=1)
    predict = predict.detach().numpy()
    abs = abs.detach().numpy()
    err = []
    for i in range(len(abs)):
        ...
        for j in range(len(predict[i])):
            ...
            x_predict += predict[i][j] * temp[0]
            y_predict += predict[i][j] * temp[1]
        err.append(sqrt((x_abs - x_predict) ** 2 + (y_abs - y_predict) ** 2))
    return err
...
class MyDataset(Dataset):
    def __init__(self, file_path, dic):
        ...
        self.site = torch.from_numpy(np.array(file.iloc[:, -2:]).astype(np.float32))

    def __len__(self):
        return len(self.x)

    def __getitem__(self, item):
        return self.x[item], self.y[item], self.site[item]
