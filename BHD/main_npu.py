...
from attack import CWAttack, FGSMAttack, AutoAttack
...
for _ in args:
    ...
    if adv_train:
        ...
        if not multi_data:
            ...
            attack_dataset = AdvDataset(
                train_batch=full_train_batch,
                attack=AutoAttack(generate_model, device)
            )
            print("Generating attack data...")
            attack_dataset = AdvDataset(
                train_batch=full_train_batch,
    ...
    if adv_test:
        ...
        for step, data in enumerate(test_batch):
            ...
            with amp.autocast():
                outputs1, outputs2 = generate_model(x)
            ...
            x_adv = fgsmattack.generate_data(x.view(len(x), -1).cpu(), grad_sign)
            x_adv = x_adv.float()
            x_adv = x_adv.view(-1, 1, len(x_adv[0])).to(device)
            bias += torch.abs(torch.abs(x_adv - x) / x).mean()
            abs_diff = torch.abs(x_adv - x)
        ...
        csv_file_name = 'test_CW_' + generate_model_type + '.csv'
        df.to_csv(csv_file_path + csv_file_name, index=False)
        print('{}-attack file is saved in {}'.format('AA', csv_file_path + csv_file_name))
        print("After {} attack, prediction's Error is {} m".format('AA', err_adv / len(test_batch)))
        print("After {} attack, ASR(threshold = {}) is {} %".format('AA', asr_threshold,
                                                                    get_asr(res_normal, res_attack,
                                                                            asr_threshold) * 100))
        print("Attack bias: {}%".format(bias / len(test_batch) * 100))
        print("Partial attack bias: {}%".format(partial_bias / len(test_batch) * 100))
